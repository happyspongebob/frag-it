...
- **V9｜2026-03-03**：AI 安慰生成接入与上线验证
  - 新增后端 `/api/comfort`：调用通义千问（DashScope OpenAI 兼容 Chat Completions），模型 `qwen-plus`，严格 JSON 输出并做稳健解析/校验
  - 前端并行请求 AI：粉碎触发时请求 `/api/comfort`，成功优先展示，失败/超时回退本地文案池
  - Vercel 配置：`DASHSCOPE_API_KEY` 覆盖 Preview/Production，并完成 Preview/Production 的 `/api/comfort` 200 返回验证
  - GitHub 治理：为 `main` 启用分支保护（Require PR + Require status checks），以 Vercel 部署检查作为合并门槛

  - 2026-03-03｜PR #1｜分支 `feat/api-comfort` → `main`｜Production｜合并后触发部署

- **V10｜2026-03-03**：生产环境体验排查与前端超时调整
  - 抓包发现生产环境 `/api/comfort` 偶发耗时接近 2.5s 导致前端 abort 回退，本地/Console 直连可成功但 UI 观感不稳定
  - 将前端 AI 请求超时从 2.5s 上调到 8s
  - 2026-03-03｜PR #2｜分支 `fix/ai-timeout` → `main`｜Production｜合并后触发部署
  - 2026-03-03｜Preview｜提交 `fix: increase ai comfort timeout to 8s`｜Preview｜部署验证
  - 复测要点：生产环境 Network 中 `/api/comfort` 可完成并返回 JSON；当请求耗时超过 2.5s 时不再因前端提前 abort 而回退

- **V11｜2026-03-08**：历史记录 / 使用日历 / Markdown 导出
  - 新增本地历史记录：粉碎完成后将结果写入 `localStorage`，可回看并持久化
  - 新增使用日历：月历视图（支持上/下月切换），按天聚合次数并以暖色深浅标记；点击日期查看当天记录；入口位于结果页且默认隐藏（点击展开）
  - 新增导出：按天生成 `YYYY-MM-DD.md`（包含 `date: YYYY-MM-DD` frontmatter），并支持 ZIP 批量导出下载
  - 新增依赖：`jszip`

- **V13｜2026-03-08**：多页面路由与日历独立页面
  - 引入 `react-router-dom` 并新增路由：`/`（主粉碎流程）、`/calendar`（使用日历页）
  - 结果页“查看使用日历”改为跳转 `/calendar`（不再使用折叠展开）
  - 新增共享历史模块/Hook：`src/history.ts`、`src/useHistory.ts`
  - 新增路由入口：`src/AppRouter.tsx`，入口文件 `src/main.tsx` 接入 `BrowserRouter`
  - 新增页面：`src/pages/CalendarPage.tsx`（月历切换、当日记录、删除/清空、zip 导出、返回）

- **V14｜2026-03-09 17:18 GMT+8**：Vercel 刷新子路由 404 修复
  - 现象：使用 `BrowserRouter` 时，刷新或直接访问 `/calendar` 会触发 Vercel `404: NOT_FOUND`
  - 修复：新增 `vercel.json` rewrites，将非 `/api` 请求重写到 `/index.html` 以支持 SPA 路由
  - 复测要点：在 `/calendar` 按 F5 刷新不应出现 404；直接访问 `https://<domain>/calendar` 应正常渲染

- **V15｜2026-03-09 17:41 GMT+8**：历史记录误清空防护与修复追踪
  - 现象：迭代/修复后用户过往记录可能不再显示，且由于数据已为空导致难以判断修复是否有效
  - 修复：读取历史时区分“读取失败/不兼容”与“确实为空”，禁止在读取失败场景将 `[]` 写回覆盖；支持旧 key 回退读取并在满足条件时迁移到新 key
  - 文档：新增 `修复文档.md` 用于记录问题现象、根因、修复策略与回归验证方法
  - 复测要点：已有记录时刷新/重载仍存在；人为制造不兼容数据时不应自动覆盖为 `[]`

## 当前版本摘要（Latest）
- **版本**：V15｜2026-03-09 17:41 GMT+8
...
### V11 功能补充
- 粉碎记录：自动写入 `localStorage`，支持单条删除与清空
- 使用日历：月历视图（支持上/下月切换），默认隐藏，结果页点击展开后可查看某天记录与次数深浅标记
- 导出：一键导出所有日期的 Markdown（按天一个文件）并打包为 zip 下载

### V13 功能补充
- 使用日历以独立页面 `/calendar` 呈现（非折叠），结果页按钮直接跳转

### 当前状态补充
- AI 文案由 `/api/comfort` 提供，生产环境已验证 200 返回；已将前端超时从 2.5s 上调到 8s 并部署，以降低误回退
